...
while the variant with `_all_` returns all found paths. The variant with
`_simple_` removes paths with cycles. The results should be the same for
simple graphs like trees, but the order of the paths may differ.
