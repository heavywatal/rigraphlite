...
test_that("attribute getters/setters work", {
  ...
  expect_s3_class({vattr = vertex_attr(g)}, "data.frame")
  expect_s3_class({vertex_attr(g) = vattr}, "data.frame")
  expect_length({edge_attr(g, "name") = g$E}, g$ecount)
  expect_equal(edge_attr(g, "name"), g$E)
  expect_s3_class({eattr = edge_attr(g)}, "data.frame")
  expect_s3_class({edge_attr(g) = eattr}, "data.frame")
})
