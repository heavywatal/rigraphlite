...
class IMatrix {
  public:
    ...
    void rownames(const cpp11::strings& x) {
      cpp11::writable::list dimnames{robj_.attr(R_DimNamesSymbol)};
      dimnames[0] = x;
      robj_.attr(R_DimNamesSymbol) = dimnames;
    }
    void colnames(const cpp11::strings& x) {
      cpp11::writable::list dimnames{robj_.attr(R_DimNamesSymbol)};
      dimnames[1] = x;
      robj_.attr(R_DimNamesSymbol) = dimnames;
    }
