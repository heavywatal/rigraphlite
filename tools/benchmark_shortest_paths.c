...
int main(void) {
  ...
  igraph_vs_t from = igraph_vss_range(sink_first, vcount);
  igraph_vs_t to = igraph_vss_range(sink_first, vcount);
  igraph_t graph;
  igraph_kary_tree(&graph, vcount, 2, IGRAPH_TREE_OUT);
  ...
  printf("igraph_distances [%d x %d]\n", sink_size, sink_size);
  igraph_matrix_t res;
  igraph_matrix_init(&res, sink_size, sink_size);
  igraph_distances(&graph, &res, from, to, IGRAPH_ALL);
  print_time();

  printf("igraph_distances [%d x 1] x %d\n", sink_size, sink_size);
  igraph_matrix_t res1col;
  igraph_matrix_init(&res1col, sink_size, 1);
  for (int i = sink_first; i < vcount; ++i) {
    igraph_distances(&graph, &res1col, from, igraph_vss_1(i), IGRAPH_ALL);
  }
  print_time();

  printf("igraph_distances [1 x %d] x %d\n", sink_size, sink_size);
  igraph_matrix_t res1row;
  igraph_matrix_init(&res1row, 1, sink_size);
  for (int i = sink_first; i < vcount; ++i) {
    igraph_distances(&graph, &res1row, igraph_vss_1(i), to, IGRAPH_ALL);
  }
  print_time();
